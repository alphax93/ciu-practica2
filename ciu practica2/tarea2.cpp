...
void MouseVentana1(int button, int state, int x, int y) {
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		printf("Ventana1, botón izquierdo presionado\n");
	}
}

void MouseVentana2(int button, int state, int x, int y) {
	if (button == GLUT_RIGHT_BUTTON && state == GLUT_UP) {
		printf("Ventana2, botón derecho liberado\n");
	}
}
...
int main(int argc, char *argv[]) {
	...
	win1 = glutCreateWindow("Práctica 2_2 Aitor Garcia Hernandez");
	InitGlew();
	Init1();
	glutDisplayFunc(Display1);
	glutMouseFunc(MouseVentana1);
	
	win2 = glutCreateWindow("Práctica 2_2 Aitor Garcia Hernandez");
	Init2();
